...
writeTrackHubSkeleton <- function(hub_dir, assembly, hub_name_string, email){
...
#' @title convert hdf5region to bigwig file
#'
#' @description This function required the executable "wigToBigWig" to be in the user's PATH 
#' 
...
#' @title Create a UCSC genome browser "Track Hub" from samplesheet. The script requires the executables
#' "wigToBigWig", "bedToBigBed" and "bigWigInfo" to be in the user's PATH
#'
#' @param samplesheet: path to the samplesheet; the samplesheet must contain a column with header sampleID, a column with header h5FullPath, or a column with header bigWigPath containing the path to the hdf5 files or the bigWig files, respectively. If the samplesheet has a column with header Peaks it must also have a column with header Tissue. Depending on the size of the hdf5 files this code might require a lot of memory
#' @param hub_name: name of the track hub; it could contain a path, in which case the path will be relative to the mountpoint. This string can be set to any value
#' @param chrom_file: path to the file containing chromosome names and lengths
#' 
#' @param assembly: genome assembly that reads were mapped to; default="hg19" 
...
samplesheetToTrackHub <- function(samplesheet, hub_name, chrom_file, assembly="hg19", mountpoint=MOUNTPOINT_PATH, http_address=MOUNTPOINT_HTTP_ADDRESS){
    ...
    writeTrackHubSkeleton(hub_dir, assembly, hub_name_string, "esterpantaleo@gmail.com")
    ...
    if (bigWigLength<2^20){
        ...
        cat(paste0("autoScale off\n",
                   "viewLimits 0:",
                   bigWigM, "\n"),
            file=file.path(assembly, "trackDbFile.txt"),
            append=TRUE)
    }else{
        cat("autoScale on\n",
            file=file.path(assembly, "trackDbFile.txt"),
    ...
    if ("Peaks" %in% colnames(samples)){
        ...
        if (peaks_files[1]!="-"){
                ...
                #write the track hub
                appendBedSuperTrack("peaks", "peaks", "peaks", bigbed_tracks, tissues, paste0(hub_name_string,tissues), "0,0,0", file=file.path(assembly_dir, "trackDbFile.txt"))
        }
    }
    printGoToMessage(hub_name, hub_dir, http_address)
}

#' @title Create a UCSC genome browser "Track Hub" from multiseq output. The script requires the executables 
...
#' @param mountpoint: path to the directory where the track hub folder will be saved in. This directory should be associated with an http address or an ftp address; default=Sys.getenv("MOUNTPOINT_PATH")
#' @param http_address: http or ftp address associated with the mountpoint; default=Sys.getenv("MOUNTPOINT_PATH")
#' @param hub_name: name of the track hub; it could contain a path, in which case the path will be relative to the mountpoint. Thisstring can be set to any value; default="multiseq"
...
multiseqToTrackHub <- function(region, chrom_file, assembly="hg19", mountpoint=MOUNTPOINT_PATH, http_address=MOUNTPOINT_HTTP_ADDRESS, hub_name="multiseq", multiseq_folder="./results_run_multiseq/"){
    ...
    if (no_bed==FALSE)
        appendBedSuperTrack("SuperBedTrack", "multiseq_signal", "multiseq signal", "multiseq_bed_file.bb", "multiseq_signal", "multiseq signal 2sd", "255,0,0", file=file.path(assembly_dir, "trackDbFile.txt"))
